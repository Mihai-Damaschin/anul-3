...
public class Main {  //открывается класс
    //инициализация логгера ошибок и исключений для вывода на консоль
    private static final Logger LOG = Logger.getLogger(Main.class.getName());

    static Scanner sc = new Scanner(System.in); //инициализация сканерра вводимых данных из консоли

    static ArrayList<Library> libList = new ArrayList<>();//инициализация списка библиотек
    static ArrayList<Book> bookList = new ArrayList<>(); //инициализация списка книг
    static ArrayList<AdvancedLibrary> advList = new ArrayList<>();
    //инициализация списка библиотек с расширенными паратметрами

    public static void main(String[] args) {
        //метод main, коорый определяет класс как "выпоняемый" для компилятора
        menu();  // вызов функции меню
    }//метод закрывается

    public static void numberOfLibsThatStartWith45() {
        //Метод для определения кол-ва библиотек, номер телефона которых начинается с 45
        int numOfLibs = 0; //инициализация кол-ва библиотек, их пока 0
        for (Library lib1 : libList) { //улучщенный цикл for по списку библиотек
            if (lib1.getTelephone().startsWith("45")) { //проверка на удовлетворение условия
                numOfLibs++; //если условие верно, кол-во библиотек увеличивается на 1
            }  //оператор if закрывается
        }// цикл закрывается
        System.out.println("Number of Libraries which phone number start with 45 = " + numOfLibs); //вывод результата в консоль
    } //метод закрывается
